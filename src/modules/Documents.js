...
var Documents = {
  ...
  getDocumentsByLabel: async function (label) {
    ...
    try {
      ...
      logger.error(`Erreur lors de la récupération des documents pour le label ${label} :`, error);
  ...
  getDocumentById: async function (id) {
    try {
      ...
      for (let i = 0; i < this.labels.length; i++) {
        ...
        if (!this.documents[label] || this.documents[label].length === 0) {
          logger.warn(`Aucun document trouvé pour le label ${label} dans le store !`); 
          continue;
        }
      ...
      logger.error(`Erreur lors de la récupération du document avec l'id ${id} :`, error);
      this.throwError(error);
    }
  },
  ...
  getFileById: async function (id) {
    try {
      ...
      logger.error(`Erreur lors de la récupération du fichier avec l'id ${id} :`, error);
      this.throwError(error);
    } finally {
      this.stopPending();
    }
